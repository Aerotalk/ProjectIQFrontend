  ...
  const handlePreview = async () => {
    try {
      ...
      const previewPayload = {
        ...
        client_name: client?.clientName || quotation.client || 'Client Name',
        client_address_line1: client?.billingAddress || '',
        client_address_line2: client?.billingCity || '',
        client_phone: client?.phone || '',
        client_state: client?.billingState || '',
        client_email: client?.email || '',
        estimate_number: quotation.qtnNo || 'Draft',
        estimate_date: quotation.date || new Date().toLocaleDateString('en-GB'),
