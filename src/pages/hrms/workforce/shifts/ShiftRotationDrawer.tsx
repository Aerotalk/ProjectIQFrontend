...
interface Props {
  ...
  initialData?: any;
  onSave: (data: ShiftRotationValues) => Promise<void>;
}

export default function ShiftRotationDrawer({ isOpen, onClose, mode, initialData, onSave }: Props) {
    ...
    <div className="fixed inset-0 z-[100] flex justify-end bg-black/20 dark:bg-black/40 transition-opacity">
      <div className="w-full max-w-md bg-white dark:bg-[#181a1f] h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
          <FormProvider {...form}>
            <form id="rot-form" onSubmit={handleSubmit(onSave)}>
              <FormLayout>
                  ...
                  <FormGrid columns={1}>
                    ...
                    <FormField label="Description" error={errors.description?.message}>
                      <textarea 
                        ...
                        disabled={readOnly} 
                        rows={3} 
                        className={`w-full bg-gray-50 dark:bg-[#1f2229] border border-gray-200 dark:border-gray-800 rounded-sm px-3 py-2 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-[#792359] focus:ring-1 focus:ring-[#792359] ${errors.description ? 'border-red-500' : ''}`} 
        ...
        <div className="px-6 py-4 border-t border-gray-200 dark:border-white/10 bg-gray-50/50 dark:bg-white/[0.02] flex items-center justify-between shrink-0">
          ...
          {!readOnly && (
            <button type="submit" form="rot-form" disabled={isSubmitting} className="flex items-center gap-2 px-6 py-2 bg-[#792359] hover:bg-[#52173c] disabled:opacity-70 text-white text-sm font-medium rounded-sm shadow-sm transition-colors">
