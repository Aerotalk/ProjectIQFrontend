    ...
    <div className="flex flex-col space-y-6 overflow-y-auto custom-scrollbar h-full pb-10">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="default" size="sm" onClick={() => navigate('../cycles')} className="bg-[#792359] hover:bg-[#b8458f] text-white">
            <Plus size={16} className="mr-2" />
            New Cycle
          </Button>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {statCards.map((stat, index) => (
          <div key={index} className="bg-white dark:bg-[#181a1f] p-4 rounded-sm border border-gray-200 dark:border-white/10 shadow-sm flex items-center space-x-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white dark:bg-[#181a1f] p-6 rounded-sm border border-gray-200 dark:border-white/10 shadow-sm">
            ...
            <div className="space-y-4">
              {mockGoals.slice(0, 3).map(goal => (
                <div key={goal.id} className="border border-gray-100 dark:border-white/5 rounded-sm p-4">
                  ...
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                    <div 
                      className={`h-2 rounded-full ${goal.progress >= 100 ? 'bg-green-500' : goal.progress >= 50 ? 'bg-blue-500' : 'bg-orange-500'}`}
          ...
          <div className="bg-white dark:bg-[#181a1f] p-6 rounded-sm border border-gray-200 dark:border-white/10 shadow-sm h-64 flex flex-col items-center justify-center">
            <TrendingUp size={48} className="text-gray-300 dark:text-gray-600 mb-4" />
            <h3 className="text-lg font-bold text-gray-700 dark:text-gray-300">Department Performance Trend</h3>
            <p className="text-sm text-gray-500 mt-2 text-center max-w-sm">
              Chart visualization will be integrated here to show performance ratings distributed by department over time.
            </p>
          </div>
        </div>
              ...
              {mockCycles.filter(c => c.status === 'Active' || c.status === 'Review Phase').map(cycle => (
                <div key={cycle.id} className="flex flex-col p-3 bg-gray-50 dark:bg-white/5 rounded-sm">
                  ...
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 mt-2">
                    <div 
                      className="h-1.5 rounded-full bg-[#792359] dark:bg-[#e6a8d0]"
