...
export const api = {
  async request(endpoint: string, options: RequestOptions = {}) {
    ...
    const shouldCache = isGet && !options.skipCache && options.responseType !== 'blob';
    ...
    if (shouldCache) {
      ...
      // 2. Check if a request for this endpoint is already in flight
      const activePromise = apiCache.getPromise(endpoint);
      if (activePromise) {
    ...
    const makeRequest = async () => {
      ...
      // On successful mutation (POST, PUT, DELETE), invalidate cache
      if (!isGet) {
        apiCache.invalidatePrefix(endpoint);
      }
    ...
    if (shouldCache) {
      ...
      apiCache.setPromise(endpoint, promise);
      return promise;
    }
